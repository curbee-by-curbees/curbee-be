...
export default class TextService {
  ...
  static async welcomeText(username, phoneNumber) {
    const text = await sendSms(
      phoneNumber,
      `Welcome to curbee, ${username}! For full customization, including adding new lookout spots, visit https://www.curbee.app.`
    );
    return text;
  } 
  ...
  static async newFindText(username, phoneNumber) {
    const text = await sendSms(
      phoneNumber,
      `Hey, ${username}! Looks like there's a new find near one of your lookout spots! For full customization, including adding new lookout spots, visit https://www.curbee.app.`
    );
    return text;
  }
}
