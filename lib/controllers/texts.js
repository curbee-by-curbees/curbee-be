  ...
  .post('/sms', async (req, res) => {
    ...
    if(firstWord.startsWith('!')) {
      ...
      //   console.log(`\x1b[35m%s\x1b[0m`,'!updatespot');
        ...
        const spotName = result.Body.split(' ')[1];
        
        const spotToDelete = await Spot.findAllByUserAndName(user.id, spotName);
        console.log(`\x1b[35m%s\x1b[0m`,'spotToDelete: ', spotToDelete);
        ...
        if(spotToDelete) {
          ...
          twiml.message(`
          \n\nWe couldn't find a lookout spot named '${spotName}'. Reply with !APPSPOTS to see your current lookout spots.`);
        }
